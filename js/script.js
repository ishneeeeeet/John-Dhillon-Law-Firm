...
(function () {
	...
	let
		...
		plugins = {
			...
			pendedImg:               document.querySelectorAll('.pended-img'),
			lazyBG:                  document.querySelectorAll('[data-lazy-bg]'),
	...
	$(function () {
		...
		// Image lazy load
		let imgLazyObserver = new IntersectionObserver( function ( entries, observer ) {
		...
		if ( plugins.lazyImg.length ) {
			plugins.lazyImg.forEach( function ( node ) {
				let
				...
				imgLazyObserver.observe( node );
			} )
		}

		// Image pended
		if ( plugins.pendedImg.length ) {
			plugins.pendedImg.forEach( function ( node ) {
		...
		// Image lazy load
		let bgLazyObserver = new IntersectionObserver( function ( entries, observer ) {
			entries.forEach( function ( entry ) {
				if ( entry.isIntersecting ) {
					entry.target.loadLazyBG();
					observer.unobserve( entry.target );
				}
			});
		})
		...
		if ( plugins.lazyBG.length ) {
			plugins.lazyBG.forEach( function ( node ) {
				node.loadLazyBG = ( function () {
					this.style.backgroundImage = `url(${ this.getAttribute('data-lazy-bg') })`;
				}).bind( node );

				bgLazyObserver.observe( node );
			} )
		}
		...
		if (plugins.owl.length) {
			for (let i = 0; i < plugins.owl.length; i++) {
				...
				if (typeof(jnode.attr("data-numbering")) !== 'undefined') {
					let numberingObject = $(jnode.attr("data-numbering"));

					jnode.on('initialized.owl.carousel changed.owl.carousel', function (numberingObject) {
						return function (e) {
